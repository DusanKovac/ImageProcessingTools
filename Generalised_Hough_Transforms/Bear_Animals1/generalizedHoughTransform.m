...
while i <= size(x_pos,1)
    ...
    %RTable_Phi calc:
    %X val
    R_Table_Phi(gradient_coe,bin_coe,1) = round(c * dx - d * dy);
    %Y val 
    R_Table_Phi(gradient_coe,bin_coe,2) = round(d * dx + c * dy);   
...
%Subplot the second bear and its accumlator matrix
...
hold on;
circle(first_bear_y, first_bear_x, 24);
hold on;
circle(first_bear_y, first_bear_x, 4);
hold on;
circle(second_bear_y, second_bear_x, 24);
hold on;
